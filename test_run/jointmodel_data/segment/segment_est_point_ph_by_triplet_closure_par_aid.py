...
def segment_est_point_ph_by_triplet_closure_par_aid(obs,Input,seg_par,seg_index ):
    ...
    #     #TODO: find lsmr alternatives
    #     point_ph(:, i)=lsmr(arc_tcp_kp, uni_arc_ph_kp_kp(:, i))
    #
    # point_ph = [point_ph(1:IDX_refpnt - 1, 1: end);zeros(1, N_arc_ph);
    # point_ph(IDX_refpnt: end, 1: end)];
    # point_ph = [obs(TCP_kp_kp, 1:5), point_ph];
...
obs = sio.loadmat('seg_est_test_data/obs_new.mat')['obs_new']
Input = sio.loadmat('seg_est_test_data/Input.mat')

import pickle
seg_par = pickle.load(open('seg_est_test_data/seg_par.pickle', 'rb'))
# seg_par = sio.loadmat('seg_est_test_data/seg_par.mat')
seg_index = 0
#################################################################################
...
arc_tcp=arc_tcp1+arc_tcp2
# del arc_tcp1, arc_tcp2, arc_index
_, st2, _=subset_detect(uni_IDX_kp[:,0],uni_IDX_kp[:,1],obs[TCP_keep,1],obs[TCP_keep,0],obs[:,1],obs[:,0],10,3)
arc_idx=st2== 1
uni_arc_ph_kp_kp=uni_arc_ph_kp[arc_idx,:]
uni_IDX_kp_kp=uni_IDX_kp[arc_idx,:]
TCP_kp_kp=np.unique(uni_IDX_kp_kp)
