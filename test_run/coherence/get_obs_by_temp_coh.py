...
def get_obs_by_temp_coh(ref_sce,rep_sce,an,rn,image_format1,image_format2,cpd,wpd,T,SHP=[]):
    ...
    #TODO: isn't ref_csv 1D? (in retrieve_obs_sb_tempcoh)
    NIFG = ref_sce.shape[0]
    os.chdir(cpd)
    ...
    if os.path.isfile('tempcoh.pickle') and os.path.isfile('MST_idx.pickle'):
        ...
        for i in range(NIFG):
            ...
            coh = read_file(coh_name, an, image_format1)
            ifgfilt_name = str(ref_sce[i]) + '-' + str(rep_sce[i]) + '.tflt.filt'
            ifgfilt = read_file(ifgfilt_name, an, image_format2)
            ifgcoh = coh * ifgfilt

            os.chdir(cpd)
            phase_name = str(ref_sce[i]) + '-' + str(rep_sce[i]) + '.tflt'
            ifg = read_file(phase_name, an, image_format2)
    ...
    for i in range(NIFG):
        ...
        obs[:, i + 5] = ifgph[ixy]
        coh_name = str(ref_sce[i]) + '-' + str(rep_sce[i]) + '.tflt.coh'
        ifgcoh = read_file(coh_name, an, image_format1)
        coh[:, i + 3] = ifgcoh[ixy]
