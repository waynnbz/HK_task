...
import numpy as np
from AdpCohEst import AdpCohEst

from OutputMat import OutputMat
...
if __name__ == '__main__':
    # load data
    mli_files = glob('data/**/*.rmli')
    diff_files = glob('data/**/*.diff')

    nlines = 501

    mlistack, mlilist = ImgRead(mli_files, nlines, 'float32')
    infstack, inflist = ImgRead(diff_files, nlines, 'cpxfloat32')


    ######## AdpCohEst TEST BLOCK ###################
    Acc=True
    Btflag=False
    SHP=None

    ###### AdpCohEst Function Section #####
    BoxCar = False
    if not SHP:
        BoxCar = True
    ...
    Coh = np.zeros((nlines, nwidths, npages), 'csingle') #csingle
    # TODO: .index() will throw if not found
    idx = [[list(mlilist.ravel('F')).index(c) for c in r] for r in inflist]
    ...
    if BoxCar:
        ...
        for ii in range(npages):
            ...
            nu_r = signal.correlate2d(np.sqrt(m1 * m2) * infstack[:, :, ii].real, h, 'same')
            nu_i = signal.correlate2d(np.sqrt(m1 * m2) * infstack[:, :, ii].imag, h, 'same')
            nu = nu_r + 1j*nu_i
            # memory saving nu = 1j*nu_i; nu += nu_r

            # denominator
            de1 = signal.correlate2d(m1, h, 'same')
            de2 = signal.correlate2d(m2, h, 'same')

            Coh[:, :, ii] = nu / (np.sqrt(de1 * de2) + np.spacing(1))

            print(f'BOXCAR estimating: {ii}/{npages} is finished...')
    ...
    ### ADP. COHERENCE ESTIMATE ###
        ...
        if Acc:
            ...
            Coh = np.reshape(Coh, (nlines * nwidths, npages), 'F')
        ...
        for ii in range(npages):
            ...
            # if ACC
                ...
                for kk in range(blocksize):
                    ...
                    # TODO: double check return coordinates, X&Y are correct
                    ...
                    temp = np.reshape(temp, (CalWin[0] * CalWin[1], num, 3), 'F') * np.tile(
            ...
            for jj in range(nwidths):
                for kk in range(nlines):
                    if remove_bias[kk, jj]:
                        ...
                        #TODO: check dim conversion maps
    ...
    ### Comparison output ###
    # OutputMat(Coh, 'data/output/D_0/CARBOX_only/Coh_py.csv')