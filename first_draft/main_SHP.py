...
from SHP_SelPoint import SHP_SelPoint
...
if __name__ == '__main__':
    ...
    # # BWS SHP selection
    # # [BWSSHP] = SHP_SelPoint(mlistack.datastack, CalWin, Alpha, 'BWS');
    ...
    if EstAgr.upper() == 'HTCI':
        ...
        for kk in range(InitCol, nwidths_EP - RadiusCol):
            for ll in range(InitRow, nlines_EP - RadiusRow):
                ...
                T = BWStest(np.tile(Ref[:, np.newaxis], (1, CalWin[0] * CalWin[1])),
    ...
    SHP = namedtuple("SHP", "PixelInd BroNum CalWin")
    SHP_obj = SHP(PixelInd, BroNum, CalWin)
