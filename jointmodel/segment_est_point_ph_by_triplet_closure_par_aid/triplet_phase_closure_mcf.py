...
def triplet_phase_closure_mcf(tri2ifg_matrix,ifg_ph_wrap,Input,correct_idx=0):
    ...
    # triplet_closure_integer = np.round(tri2ifg_matrix @ ifg_ph_wrap / (2 * np.pi))
    # banker round
    triplet_closure_integer = np.vectorize(lambda x: np.ceil(x) if x % 1 >= 0.5 else np.round(x))(
        tri2ifg_matrix @ ifg_ph_wrap / (2 * np.pi))
    ...
    if correct_idx == 1:
        ...
        # update intv phase for arc with zero closure
        ...
        arc_interval_ph[:, unwrap_correct_idx_partial_2] = \
            np.linalg.lstsq(B, ifg_ph_partial[:, unwrap_correct_log_partial_1], rcond=None)[
                0]  # B\ifg_ph_partial[:, unwrap_correct_log_partial_1]
