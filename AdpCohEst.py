...
def AdpCohEst(mlistack, mlilist, infstack, inflist, SHP=False, Btflag=False, Acc=True):
    '''
    ...
    # TODO: optimizable with np.unravel_index #list comprehension with reshape
    idx = [[list(mlilist.squeez()).index(c) for c in r] for r in inflist]

    idx = np.zeros(inflist.shape)
    idx_flag, idx_squeeze = ismember(inflist, mlilist)
    _iii = 0
    for _ii, _vv, in np.ndenumerate(idx_flag):
        if _vv:
            idx[_ii] = idx_squeeze[_iii]
            _iii += 1
        else:
            idx[_ii] = -1
        ...
        for ii in range(npages):
            ...
            # if ACC
                ...
                for kk in range(blocksize):
                    ...
                    # Y, X = ind2sub([nlines, nwidths], block_size(kk, 1):block_size(kk, 2)) #inverse shape
                    X, Y = np.unravel_index(np.arange(block_size[kk, 0], block_size[kk, 1]), (nwidths, nlines))
                    Y = Y + RadiusRow
                    X = X + RadiusCol

                    for jj in range(num):
                        temp[:, :, :, jj] = m1[Y[jj] - RadiusRow:Y[jj] + RadiusRow, X[jj]-RadiusCol:X[jj] + RadiusCol,:]
